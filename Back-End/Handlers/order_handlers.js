...
function setupOrderHandler(router, dbConnection) {
  ...
  router.post('/', verifyJWTMiddleware(jwtUtil), async (request, response) => {
    ...
    try {
      ...
      const sqlCheck =
        "SELECT id, laptop_id, quantity  FROM cart_table WHERE user_id = ? AND status = 'pending'";
      ...
      if (result.length > 0) {
        result.forEach((row) => {
          idArray.push(row.id), laptop_idArray.push(row.laptop_id);
          quantityArray.push(row.quantity);
        });

        // Mulai transaksi
        await dbConnection.beginTransaction();
        const sqlUpdateLaptop =
          'UPDATE laptop_table l INNER JOIN cart_table c ON l.id = c.laptop_id SET l.quantity = CASE WHEN (l.quantity - c.quantity) < 0 THEN l.quantity ELSE (l.quantity - c.quantity) END WHERE c.laptop_id IN (?) AND l.quantity >= c.quantity';

        const resultUpdateLaptop = await dbConnection.query(sqlUpdateLaptop, [
          laptop_idArray,
        ]);
        ...
        const sqlGetTotalPrice =
          "SELECT (l.price * c.quantity) AS price FROM cart_table c JOIN laptop_table l ON c.laptop_id = l.id WHERE c.user_id = ? AND c.status = 'pending'";
        const [resultGetTotalPrice] = await dbConnection.query(
          sqlGetTotalPrice,
          data[0]
        );
        ...
        const sqlUpdateCart =
          "UPDATE cart_table SET status = 'finished' WHERE user_id = ? AND status = 'pending'";
        const resultUpdateCart = await dbConnection.query(
          sqlUpdateCart,
          data[0]
        );
        ...
        const sqlInsert =
          'INSERT INTO orders_table (user_id, cart_id, status, laptop_id, quantity, total_price) VALUES (?,?,?,?,?, ?)';
        const values = [
          data[0],
          joinedResultCartId,
          order_status,
          joinedResultLaptopId,
          joinedResultQuantity,
          priceAccumulation,
        ];
  ...
  router.get('/', verifyJWTMiddleware(jwtUtil), async (request, response) => {
    ...
    try {
      const sqlGetId =
        "SELECT id, cart_id, laptop_id, quantity, total_price, DATE_FORMAT(order_date, '%Y-%M-%D') AS order_date, status FROM orders_table WHERE user_id = ?";
      ...
      for (const row of resultGetId) {
        ...
        for (let i = 0; i < dataLaptopId.length; i++) {
          ...
          const sqlGetData =
            'SELECT name, price, image AS picture FROM laptop_table WHERE id = ?';
  ...
  router.put('/', verifyJWTMiddleware(jwtUtil), async (request, response) => {
    const data = [request.body.orderId, request.body.userId];
    ...
    try {
      ...
      const sqlGetData =
        'SELECT * FROM orders_table WHERE id = ? AND user_id = ? AND status = ?';
      ...
      const sqlChangeStatus =
        "UPDATE orders_table SET status = 'finished' WHERE id = ? AND status = 'ongoing' ";
  ...
  router.get('/accumulation-price', async (request, response) => {
    ...
    try {
      // Query untuk mengambil akumulasi harga per tanggal
      const sqlGetPayment = `
            SELECT 
                DATE_FORMAT(order_date, '%Y-%M-%D') AS order_date,
                SUM(total_price) AS price_accumulation
            FROM 
                orders_table
            GROUP BY 
                DATE(order_date)
      ...
      // Eksekusi query
      const [result] = await dbConnection.query(sqlGetPayment);

      // Array untuk menampung data hasil
      const insight = [];

      // Loop melalui setiap baris hasil dan tambahkan ke array insight
      result.forEach((row) => {
        const paymentInfo = {
          orderDate: row.order_date,
          priceAccumulation: row.price_accumulation,
        };
        insight.push(paymentInfo);
      });

      // Kirim respons JSON setelah selesai melooping
      response.json({
        status: true,
        message: 'Data retrieved successfully',
        Result: insight,
