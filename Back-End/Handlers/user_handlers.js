  ...
  router.post('/signup', async (request, response) => {
    ...
    try {
      ...
      if (!isEmailRegistered) {
        ...
        const isPasswordMatch = rows.some((row) =>
          hashFunction.compare(data[2], row.password)
        );

        const sql =
          "INSERT INTO users_table (name, email, password, address, phone_number, role) VALUES (?,?,?,?,?, 'user')";
        const value = [
          data[0],
          data[1],
          hashFunction.hash(data[2]),
          data[3],
          data[4],
        ];
    ...
    async (request, response) => {
      ...
      if (dataRole == 'admin') {
        try {
          ...
          const dataCek = [request.body.email, request.body.name];
          if (dataCek[0]) {
            sql =
              'SELECT email, name, phone_number, address, role FROM users_table WHERE email = ?';
            data.push(dataCek[0]);
          } else if (dataCek[1]) {
            sql =
              'SELECT email, name, phone_number, address, role FROM users_table WHERE name LIKE ?';
            data.push(`%${dataCek[1]}%`);
          } else {
            sql =
              'SELECT email, name, phone_number, address, role FROM users_table';
          }
    ...
    async (request, response) => {
      ...
      if (dataRole == 'user' && dataId == getId) {
        try {
          ...
          const sql =
            'SELECT email, name, phone_number, address, role FROM users_table WHERE id = ?';
  ...
  router.put('/', verifyJWTMiddleware(jwtUtil), async (request, response) => {
    ...
    if (dataId == data[0] || dataRole == 'admin') {
      try {
        ...
        if (rows.length > 0) {
          const sql =
            'UPDATE users_table SET name = ?, address = ?, phone_number = ? WHERE id = ? ';
